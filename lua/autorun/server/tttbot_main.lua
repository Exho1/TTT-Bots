...
function generatenavmesh()
	local ply = table.Random( player.GetBots() )
	...
	if(HitWorld) then
		...
		if(IsValid(ply)) then
			...
			navmesh:SetupMaxDistance(ply:GetPos(), 1024) -- All nodes must stay within 256 vector distance from the players position
		end
		...
		--ErrorNoHalt("Generated in "..string.ToMinutesSeconds(navmesh:FullGeneration()).."\n")
	end
end

concommand.Add("path2", function()
	local ply = table.Random( player.GetBots() )
	
	--ply:generateNav( function()
		print("Done!")
		ply:generatePathTo( findExho():GetPos() )
	--end)
